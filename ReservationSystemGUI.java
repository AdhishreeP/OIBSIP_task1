...
public class ReservationSystemGUI {
    ...
    try {
        ...
        if (seatNumber < 1 || seatNumber > 10) {
            ...
            JOptionPane.showMessageDialog(frame, "Seat " + seatNumber + " is already reserved! Please choose another seat.");
        } else {
    ...
    try {
        int seatNumber = Integer.parseInt(seatNumberTextField.getText().trim());
        if (seatNumber < 1 || seatNumber > 10) {
            JOptionPane.showMessageDialog(frame, "Please enter a seat number between 1 and 10.", "Invalid Seat Number", JOptionPane.ERROR_MESSAGE);
        } else if (!seats[seatNumber - 1]) {
            JOptionPane.showMessageDialog(frame, "Seat " + seatNumber + " is not currently reserved.", "Seat Not Reserved", JOptionPane.ERROR_MESSAGE);
        } else {
            seats[seatNumber - 1] = false;
            JOptionPane.showMessageDialog(frame, "Reservation for seat " + seatNumber + " has been cancelled.", "Reservation Cancelled", JOptionPane.INFORMATION_MESSAGE);
            seatNumberTextField.setText("");
        }
    } catch (NumberFormatException ex) {
        JOptionPane.showMessageDialog(frame, "Please enter a valid seat number.", "Invalid Seat Number", JOptionPane.ERROR_MESSAGE);
    }
}
